...
Inspired from [[https://github.com/karthink/.emacs.d/blob/17959a61bb18758f55eb27b3520d173d69346353/lisp/setup-windows.el][here]]
#+begin_src emacs-lisp
...
The goal is to have the posibility to run shell command from a org mode buffer

Need to install dbus and dtach
#+begin_src sh
  brew install dbus dtach
#+end_src
...
This code block create an export on top of the gfm org babel export.
It adds the hide button for src and heddings.
#+begin_src emacs-lisp
...
For python I use ~uv~ to handle the python version and virtual env.
#+begin_src emacs-lisp
  (use-package python
    :config
